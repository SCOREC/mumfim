...
#include <cstring>
  ...
  namespace Biotissue {
    ...
    int MicroFO::main_solver(
      ...
      double & local_S11, double & local_S12, double & local_S13,
                          double & local_S22, double & local_S23,
                                              double & local_S33,
      double * loc_dSdx,
      double & loc_vastrx, double & loc_vastry, double & loc_vastrz,
      double & fem_res_norm)
    {
      int result = 0;
	...
	std::ofstream out_file;
        out_file.open("results.dat",std::ofstream::app);
        double stiffness = calc_stiffness();
        out_file << fiber_network->sideCoord(FiberNetwork::RIGHT)
		 << " " << -stiffness << std::endl;
        out_file.close();
        //out_file << fiber_network->CUBE_RIGHT <<" "<< -stiffness << " " << -local_S11 << std::endl ;
        //out_file.close();
      }
	 ...
	 Volume averaged stresses are dimensionalized by multiplying by scale_conversion.
      ...
      double dir[3];
      calcAvgFiberDirection(*fiber_network,dir);
      rve_info[4*3*6+9  ] = dir[0];
      rve_info[4*3*6+9+1] = dir[1];
      rve_info[4*3*6+9+2] = dir[2];
	...
	const Node & node = fiber_network->node(n);
	 ...
	 // extract out only forces on x faces
	
	if (node.y != 0.5 && node.y != -0.5 && node.z != 0.5 && node.z != -0.5){
	  if (node.x > 0)
	  {
	    fxp += force_vector[dof];
	  }
	  else
	  {
	    fxn += force_vector[dof];
	  }    
	}
	...
	if (!TRUSS) //beam
	{
      ...
      stress[5] = zfz;
/*
      AMSI_DEBUG(std::cout << "Microscale stress: " << stress[0] << " " << stress[1] << " "
                                                    << stress[2] << " " << stress[3] << " "
                                                    << stress[4] << " " << stress[5] << std::endl);

      AMSI_DEBUG(std::cout<< "fxp = " << fxp << std::endl
		          << "fxn = " << fxn << std::endl);
*/
    }
					  ...
					  double * stress)
    {
      ...
      //zero(lu_decomp_matrix);
      //zero(lu_decomp_cols);
      //zero(lu_decomp_rows);
      //zero(double_work_array);
      //zero(int_work_array);
...
//      make_dRVEdFE(dxrdx,coords);  
    ...
    double MicroFO::compute_total_fiber_length()
    {
      double result = 0.0;
	...
	const Node & n1 = fiber_network->node(e.node1_id);
	const Node & n2 = fiber_network->node(e.node2_id);

	double length = pow((n2.x - n1.x),2) + 
                        pow((n2.y - n1.y),2) + 
	                pow((n2.z - n1.z),2);
