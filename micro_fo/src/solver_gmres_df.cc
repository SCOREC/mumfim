...
namespace Biotissue {
  ...
  int MicroFO::Solver()
  {
	...
	ncv = 1;
      ...
      {
	tolerance = 1e-10;
	AMSI_DEBUG(std::cout<<"step = "<<step<<", norm = "<<norm<<std::endl;)
      }
      else if(step == 80)
      {
	tolerance = 1e-8;
			   ...
			   bool calc_dSdy)
{
  ...
  if(!calc_dSdy)
  {
    ...
    /* Affinely deform supported fibers if SPECIFY_FIBER_TYPE is true */
    SupportFiberNetwork * spfn = dynamic_cast<SupportFiberNetwork*>(fiber_network);
    if (spfn)
    {
      int num_support_nodes = spfn->numSupportNodes();
