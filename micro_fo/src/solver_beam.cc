  ...
  void MicroFO::calc_matrix_beam(double * matrix,double * matrix_axial,double * lengths)
{
  ...
  for(int ii = 0; ii < num_elements; ii++)
  {
    ...
    double dL = 1.0/L;
    double E = fiber_get_E(ii);
    double A = fiber_network->fiberArea();
...
void MicroFO::make_material_stiffness_matrix(double * matrix, int ielem, double L)
{
    ...
    double E = fiber_get_E(ielem);
    double A = fiber_network->fiberArea();
    double G = fiber_get_G(ielem);
    double J = fiber_get_J(ielem);
    double I1 = fiber_get_I1(ielem);
    double I2 = fiber_get_I2(ielem);
