...
// scorecutil stuff to be taken out eventually
...
namespace Biotissue {
  ...
  void MicroFO::create_element_shape_vars(double & vol,
					  ...
					  double * fedisp)
    {
      ...
      //double x,y,z;
      //int i,j,kj,k,n;
      double/* gp[2],gw[2],x1[8],y1[8],z1[8],*/ddett[24];
      double dett;
...
*/  
      /* manually modify rvedisp
	 set vertices 0, 2, 4, and 6 of rve to be (0,0,0).
      ...
      AMSI_DEBUG(std::cout <<"modified rvedisp:"<<"("<<rvedisp[0]<<","<<rvedisp[1]<<","<<rvedisp[2]<<")"
                        	                <<", ("<<rvedisp[3]<<","<<rvedisp[4]<<","<<rvedisp[5]<<")"
                        	                <<", ("<<rvedisp[6]<<","<<rvedisp[7]<<","<<rvedisp[8]<<")"
                        	                <<", ("<<rvedisp[9]<<","<<rvedisp[10]<<","<<rvedisp[11]<<")"
                        	                <<", ("<<rvedisp[12]<<","<<rvedisp[13]<<","<<rvedisp[14]<<")"
...
//	if (SPECIFY_FIBER_TYPE)  
  ...
  void MicroFO::make_dRVEdFE(double * dRVEdFE, double * lcoords)
  {
    ...
    */
    double scalex = 2 * HALF_RVE_DIM; 
    double scaley = 2 * HALF_RVE_DIM;
    double scalez = 2 * HALF_RVE_DIM;
    ...
    double ex =  HALF_RVE_DIM;
    double ey =  HALF_RVE_DIM;
    double ez = -HALF_RVE_DIM;
	...
	for(int kk = 0; kk < 2; kk++)
	{
	  ...
	  for(int ii = 0; ii < num_rve_doubles / 3; ii++)
	  {
	    ...
	    dRVEdFE[(3*count)   * num_rve_doubles + ii*3    ] = diff / scalex;
	    dRVEdFE[(3*count+1) * num_rve_doubles + ii*3 + 1] = diff / scaley;
	    dRVEdFE[(3*count+2) * num_rve_doubles + ii*3 + 2] = diff / scalez;
	  }
