...
//        current_file << fiber_network_filename << ".txt";
    ...
    MPI_Datatype micro_fo_header_data_type = mdt.micro_fo_header_data_type;
    MPI_Datatype micro_fo_parameter_data_type = mdt.micro_fo_parameter_data_type;
    ...
    // Multiscale data
    std::vector<micro_fo_header> headers;
    std::vector<micro_fo_params> params;
    ...
    while(!sim_complete)
    {
      ...
      cs->Communicate(recv_pattern_init_id,
                      headers,
                      micro_fo_header_data_type);
      cs->Communicate(recv_pattern_init_id,
                      params,
                      micro_fo_parameter_data_type);
      cs->Communicate(recv_pattern_init_id,
                      initial_data,
      ...
      for(currentRVE = rves.begin(); currentRVE != rves.end(); ++currentRVE)
      {
        if(*currentRVE == NULL)
        {
          // Construct new RVE
          double pt[3];
          micro_fo_header & hdr = headers[ii];
          micro_fo_params & prms = params[ii];
          micro_fo_init_data & init = initial_data[ii];
...
#       if defined(LOGRUN) && defined(SCOREC)
      ...
      if(rves.size() > 0 && step_iter[0] % 100 == 0)
      {
        std::stringstream output_file;
        output_file << amsi::fs->getResultsDir() << "fn_rnk_" << local_rank << "_stp_" << step_iter[0]+1 << ".txt";
...
#       ifdef LOGRUN
      ...
      filename << amsi::fs->getResultsDir()
               << "micro_fo_efficiency." << local_rank << ".log";
      {
        std::fstream fs(filename.str().c_str(), std::ios::out | std::ios::app);
        amsi::flushToStream(micro_fo_efficiency,fs);
      }
      filename.str("");
      filename << amsi::fs->getResultsDir()
               << "micro_fo_weights." << local_rank << ".log";
      {
        std::fstream fs(filename.str().c_str(), std::ios::out | std::ios::app);
        amsi::flushToStream(micro_fo_weights,fs);
      }
      filename.str("");
      filename << amsi::fs->getResultsDir()
               << "micro_fo_timing." << local_rank << ".log";
      {
        std::fstream fs(filename.str().c_str(), std::ios::out | std::ios::app);
        amsi::flushToStream(micro_fo_timing,fs);
      }
#       endif
      // adaptation
      //cs->Relation_Broadcast(cplng,adapting,MPI_INTEGER);
    } // done
#   ifdef LOGRUN
...
#   endif
  ...
  void initCoupling(size_t & recv_pattern_id, size_t & send_pattern_id)
  {
    amsi::ControlService * cs = amsi::ControlService::Instance();
                ...
                double ddett[])
  {
    ...
    *dett = x1 * (y2 * z3 - y3 * z2) - y1 * (x2 * z3 - x3 * z2) + z1 * (x2 * y3 - y2 * x3);
    ...
    {
      ddett[in] = dx1[in] * (y2 * z3 - y3 * z2) -
        y1 * (dx2[in] * z3 - dx3[in] * z2) +
        z1 * (dx2[in] * y3 - y2 * dx3[in]);
      ddett[in + 1] = x1 * (dy2[in + 1] * z3 - dy3[in + 1] * z2) -
        dy1[in + 1] * (x2 * z3 - x3 * z2) +
        z1 * (x2 * dy3[in + 1] - dy2[in + 1] * x3);
      ddett[in + 2] = x1 * (y2 * dz3[in + 2] - y3 * dz2[in + 2]) -
        y1 * (x2 * dz3[in + 2] - x3 * dz2[in + 2]) +
        dz1[in + 2] * (x2 * y3 - y2 * x3);
    }
  }
                               ...
                               double * dfdl)
  {
    ...
    for(int ii = 0; ii < num_elements; ii++)
    {
      ...
      const FiberReaction * fiber_reaction = fiber_types[fiber.fiber_type];
