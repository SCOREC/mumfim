...
namespace las
{
  ...
  public:
    ...
    inline int getNumEqs() const { return neq; }
    inline int getNumNonzero() const { return nnz; }
    inline int operator()(int rw, int cl) const
    {
      int result = -1;
      int fst = rws[rw] - 1; // 1-indexing    v also here
      while((fst < rws[rw+1] - 2) && (cls[fst] - 1 < cl))
        ++fst;
      // assumes row isn't empty
    ...
    inline int * getRows() { return &rws[0]; }
    inline int * getCols() { return &cls[0]; }
  };
  /**
