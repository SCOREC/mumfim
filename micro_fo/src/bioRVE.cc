...
// get the only mesh entity
    ...
    element_mesh->end(it);
    macro_element = apf::createMeshElement(element_mesh, macro_entity);
  }
  MicroFO::~MicroFO()
  {
...
// Avoid memcpy in above function
  ...
  void MicroFO::collectMigrationData()
  {
    ...
    for(int ii=0;ii<8;ii++)
      >>>>>>> incremental debugging and adding a ton of new features taken from the micro/ version of the microscale code which saw initial development but never full adoption into the multi-scale codebase
      {
        memcpy(dSdx, rve_info + 9, 6 * num_rve_doubles * sizeof(double));
      }
    void MicroFO::collectMigrationData()
