...
namespace bio
{
  ...
  class ExplicitAnalysisBase
  {
    ...
    bool run()
    {
      ...
      std::cout << "This problem has: " << nnds << " nodes and " << nelem
                << " elements\n";
      std::cout << "Using Deformation Gradient BC has fixed " << disp_nfixed
                << " of " << ndof << " degrees of freedom.\n";
      ...
      std::cout
          << "The initial timestep is dt=" << dt_crit
          << ".\nIf the analysis continues with a comprable dt the analysis "
             "will take approximately "
          << (unsigned long)(total_time / (dt_crit * crit_time_scale_factor))
          << " timesteps.\n";
      ...
      std::cout << "Analysis has successfully completed in " << n_step
                << " timesteps.\n";
      return true;
    }
                                    ...
                                    SH_RODA>
  {
                               ...
                               mass_field)
    {
      ...
      f_int_d = createDeviceDoubleArray(ndof);
  ...
  bool FiberRVEAnalysisExplicit::run(const DeformationGradient & dfmGrd)
  {
    ...
    {
      // run serial analysis
      std::cout << "Running CPU Analysis" << std::endl;
      ExplicitAnalysisSerial analysis(
          ...
          getFn()->getVField(), getFn()->getAField());
    ...
    {
      // run gpu analysis
      std::cout << "Running GPU Analysis" << std::endl;
