...
namespace bio
{
  ...
  void ExplicitTrussIntegrator::inElement(apf::MeshElement * me)
  {
    TrussIntegrator::inElement(me);
    fe_damp.resize(es->nedof());
    fe_damp.zero();
    sound_speed = sqrt(fr->E / fr->fiber_density);
    auto ops = las::getLASOps<las::MICRO_BACKEND>();
    ops->get(v, es->nedof(), &es->dofs(0), &vel_arr);
  }
  void ExplicitTrussIntegrator::atPoint(const apf::Vector3 &, double, double)
  {
    auto f_dfdl = fr->forceReaction(lo, l);
    double f = f_dfdl.first;
    // the scalar version of the force derivative
    ...
    for (int ii = 0; ii < 2; ii++)
    {
      ...
      for (int jj = 0; jj < 2; jj++)
      {
        ...
        for (int kk = 0; kk < dim; kk++)
        {
          ...
          {
            // es->ke(ii*dim + kk, jj*dim + ll) += rctn[kk][ll] * op;
            fe_damp(ii * dim + kk) += rctn[kk][ll] * op *
                                      vel_arr[jj * dim + ll] *
  ...
  void ExplicitTrussIntegrator::outElement()
  {
    delta_t_crit_elmt = l / sound_speed;
    if (delta_t_crit_elmt < delta_t_crit)
    {
    ...
    ops->assemble(f, es->nedof(), &es->dofs(0), &es->fe(0));
    ops->assemble(f_damp, es->nedof(), &es->dofs(0), &fe_damp(0));
    delete[] vel_arr;
    amsi::destroyApfElementalSystem(es);
    apf::destroyElement(elmt);
  }
