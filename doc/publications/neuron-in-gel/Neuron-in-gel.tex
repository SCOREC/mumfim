...
Upon solving for network equilibrium in each RVE, the Cauchy stress tensor of the microscopic scale is computed from the forces of fibers that lie on the boundary of the RVE (bcl) \citep{Chandran:2007hy,Stylianopoulos:2007dp}
%
\begin{equation}
...
\end{equation}
%
The procedure in Eq.\ \eqref{eq:macro_stress_discrete} is referred to as \textit{upscaling}. The Cauchy stress tensor obtained from the microscopic scale is used to solve the macroscopic force balance \citep{Chandran:2007hy,Stylianopoulos:2007dp}
%
\begin{equation}
...
\input{figure/multi-scale-nonlinear-iteration.pdf_tex}\label{fig:nonlinear-iteration}
\input{figure/multi-scale-coupling-data.pdf_tex}

Our model is implemented using the adaptive multi-scale simulation infrastructure (AMSI) \citep{Tobin:2017ip}, which is designed to support massively parallel simulations on multiple scales. AMSI enables our model to accommodate the high computational demands of solving the mechanical equilibrium equations on the complex geometry of the neuron structure and for the multi-scale problem in the collagen gel. 
%=========================================================================================================

AMSI provides features to combine existing single-scale analysis codes into novel multi-scale analyses. 

The macroscopic simulation code and the microscopic simulation codes are decoupled save for their interactions through the AMSI libraries. AMSI manages the parallel execution space for each code, as well as the parallel communication required for the transmission of required data between the scales. Each of the scales in the simulation supplies information required to inform the requirements of the multi-scale communication and the distribution of inter-scale coupling data in  both the parallel execution space and the problem domain. This data is used by the AMSI multi-scale communications systems to communicate the coupling data required at each phase of simulation, and accomodate dynamic run-time operations such as load balancing while maintaining the coupling communication model. A discussion of the underlying design consideration of AMSI can be found in \citep{Delalondre:2010kt}.
...
In the macro-scale simulation the unstructured mesh is partitioned across the parallel execution space of the scale, the distribution of the mesh and the order of the continuity of the displacement field inform the choice of numerical integration points, at which a micro-scale simulation is to take place (for specified regions). 

Using the AMSI library, the unrelated micro-scale and macro-scale simulation codes are combined into a single simulation. Figure \ref{fig:nonlinear-iteration} illustrates the parallel execution of a sinlge nonlinear iteration of the multi-scale simulation with respect to the individual scales. While each scale is idle during a section of each multi-scale nonlinear iteration, 
