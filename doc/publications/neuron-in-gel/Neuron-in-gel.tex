...
$
\end{center}
\caption{\label{fig:image_to_model} (a) Stack of confocal images of a neuron from neuronal culture. (b) Model of neuron embedded in collagen gel that is generated using the \textit{ImageToModel} tool \cite{Klaas_chapter, Klaas_conference, simmetrix} from Simmetrix Inc. Regions in the neuron that are marked with dashed rectangles are the cell bodies, while those that are not marked constitute the axons of the neuron. The region enclosing the neuron is the collagen gel. (c) Cut in neuron-in-gel model showing the mesh generated using the SimModeler tool \cite{simmetrix,Shephard:2000vc} from Simmetrix Inc. A mesh of $\sim$80k tetrahedral elements is used for simulations in this work.}
\end{figure}
%

The confocal images are stacked together and converted to a 3D voxelated data set using \textit{ImageJ} \cite{Schneider:2012dw}, where each voxel has dimensions of 0.22$\mu$m $\times$ 0.22$\mu$m $\times$ 2$\mu$m. The voxelated data is converted to a discrete geometric model and its corresponding unstructured mesh using tools of Simmetrix Inc. \cite{simmetrix,Shephard:2000vc}.  Specifically, the voxelated data of the neuron is converted to a discrete geometric model using the \textit{ImageToModel} tool \cite{Klaas_chapter, Klaas_conference, simmetrix}. Subsequently, the geometric model of the neuron structure is enclosed in a box that represents the domain of the collagen gel, where the neuron-gel interface is perfectly bonded. The mesh of the embedded neuron structure was generated using the SimModeler tool \cite{simmetrix,Shephard:2000vc}. The neuron-in-gel model and its corresponding mesh is shown in Figs.\ \ref{fig:image_to_model}(b) and (c), respectively. 
...
\end{equation}
%
where $f$ is the probability density function of the maximum principal strain (MPS) normalized by the bulk strain that is applied to the surrounding gel, $\epsilon_{\text{max}}/\epsilon_{\text{bulk}}$, and $X$ is a specific value of the normalized MPS. A normalized MPS that is greater than unity indicates that the applied bulk strain on the surrounding collagen gel is amplified in the neuron structure. Such local-strain amplification can give rise to injury in the neuron even when moderate loads are applied to the surrounding gel. Equation \ref{eq:ccdf} is the probability that the neuron structure (equivalent to the volume fraction of the neuron) experiences a local-strain amplification of \textit{at least} $X$. 
...
\end{figure}
%
For both non painful and painful ligament loading, the average normalized MPS in the neuron structure is greater than unity. In both cases, portions of the neuron experience local-strain amplification of more than three times the applied bulk strain on the surrounding gel. These results quantitatively capture what is observed in the color maps of Fig.\ \ref{fig:neuron_0deg_MPS}. 

%=========================================================================================================
\subsection{Effect of Changing the Axial Stiffness in the Cell Body}

In our model, the axial stiffness of the cell body depends on how quickly the microtubules in the neuron transition from an aligned state in the axon to an unaligned state in the cell body. The transition from the aligned to unaligned state is characterized by the variable $a$ in Eq.\ \eqref{eq:cellEA}. To explore the effect of such a transition on the overall mechanical behavior of the neuron, the value of $a$ in Eq.\ \eqref{eq:cellEA} is adjusted. A larger value of $a$ represents a more rapid transition from the aligned to unaligned state and results in a softer cell body in the axial direction. 

As seen in Eq.\ \eqref{eq:cellEA}, the value of $a$ needs to be greater than $D$, which is 25.5 $\mu$m in order for $E_A^{\text{cell}}$ to always be greater than zero. In this section three values of $a$ are considered: $a=$ 28 $\mu$m, 30 $\mu$m, and 50 $\mu$m, which correspond to axial stiffnesses of approximately 6.25 kPa, 10.5 kPa, and 34.3 kPa, respectively, at the center of the largest cell body in the neuron. The ccdfs for different axial stiffnesses of the cell body (different values of $a$) are plotted in Fig.\ \ref{fig:neuron_ccdf_vary_a} for bulk strain of 16$\%$ in the collagen gel and loading at 0 degrees.
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Discussion}
To explain the trends seen in the results of Section \ref{sec:Results}, two opposing contributions that arise when a neuron that is embedded in a collagen gel is deformed.
%
\begin{enumerate}
\item The axial direction of the embedded neuron rotates to align with the direction of loading.
\item The stiffness of the gel increases (see stress-strain curve in Fig.\ \ref{fig:fiber_param}(a)).
\end{enumerate}
%
Contribution 1 causes the neuron to become stiffer relative to the surrounding gel because the neuron structure is significantly stiffer in the axial direction. The alignment of the neuron is driven by the lateral forces that arise from the Poisson effect of the surrounding collagen gel. Therefore, a larger Poisson effect (i.e., larger Poisson ratio) will give rise to more rapid alignment of the neuron structure. In contrast, contribution 2 causes the neuron to become softer relative to the surrounding gel. When contribution 1 is greater than contribution 2, the local-strain amplification decreases as the surrounding gel is increasingly deformed. This behavior is observed along axis 1 for 0 and 45 degree loading (see Fig.\ \ref{fig:axial_amp}(a) and (b)). In the opposite scenario where contribution 2 is greater than contribution 1, the local-strain amplification increases as the surrounding gel is increasingly deformed. This behavior is observed in axis 2 for 90 degree loading (see Fig.\ \ref{fig:axial_amp}(f)). 

The size of the change in local-strain amplification depends on the relative dominance of each contribution. The small variations seen in the distributions of axial strain along axis 1 for 0 and 45 degree loading (Figs.\ \ref{fig:axial_distr}(a) and (b)) indicate that contributions 1 and 2 are nearly equal - the amount of stiffening in the neuron structure due to its relative alignment with respect to the loading direction is similar to the stiffening of the surrounding collagen gel. On the contrary, the large variations seen in the distributions of axial strain along axis 2 for 90 degree loading where the distributions shift right towards larger local-strain amplification (e.g., see Fig.\ \ref{fig:axial_distr}(f)) indicates that contribution 2 is significantly more dominant than contribution 1 - the amount of stiffening in the neuron structure due to its relative alignment with respect to the loading direction is significantly smaller than the stiffening of the surrounding collagen gel.

Two configurations of the neuron structure can be identified from the analysis above. In configuration 1, the neuron structure is nearly parallel to the direction of loading and contributions 1 and 2 are similar. In configuration 2, the neuron structure is nearly orthogonal to the direction of loading and contribution 2 is dominant over contribution 1. Physiologically, configuration 1 is preferred because local-strain amplification decreases as the surrounding collagen gel is increasingly deformed. On the contrary, configuration 2 will give rise to neuronal damage even when the surrounding collagen gel is moderately loaded.
 
Based on the ideas developed above for the axial strains in the neuron region selected in Fig.\ \ref{fig:axial_schematic}, the increase in local-strain amplification seen in the entire neuron structure for 90 degree loading (see Fig.\ \ref{fig:neuron_ccdf_mps}(c)) is due to the fact that a majority of the neuron lies in configuration 2, while the small variations in the distribution and decrease in local-strain amplification for 0 degree loading (see Fig.\ \ref{fig:neuron_ccdf_mps}(a)) is due to the fact that a majority of the neuron lies in configuration 1.
