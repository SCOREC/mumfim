...
$
\begin{array}{ccc}
\includegraphics[height=1.5cm]{figure/confocal-2.pdf} & 
\includegraphics[height=1.5cm]{figure/confocal-3.pdf} &
\includegraphics[height=1.5cm]{figure/confocal-4.pdf} \\
\end{array}
$
\end{center}
\caption{\label{fig:Z-stack-images} Images taken from Zeiss LSM 510 confocal microscope.} 
\end{figure}
%
...
$
\end{center}
\caption{\label{fig:image_to_model} \blue{Show voxelated and discrete geometry of neuron (no gel) so that full pipeline can be visualized.} (a) Stack of confocal images of a neuron structure from neuronal culture. (b) Model of neuron structure embedded in collagen gel that is generated using the \textit{ImageToModel} tool \citep{Klaas:2013ug, Klaas_conference, simmetrix} from Simmetrix Inc. The \red{neuron structure} consists of three neurons and the three cell bodies within the neuron structure are labeled with their axial length - axons are unlabeled. The region enclosing the neuron structure is the collagen gel. (c) Cut in neuron-in-gel model showing the mesh generated using the SimModeler tool \citep{simmetrix,Shephard:2000vc} from Simmetrix Inc. A mesh of $\sim$80k tetrahedral elements is used for simulations in this work.}
\end{figure}
%
...
The stiffness in tension and compression in the linear regimes of the fiber force are 29.7 kPa and 1.65 kPa, respectively, for the values listed in Table \ref{table:multiscale_parameters}. As mentioned above, the significantly smaller stiffness in compression relative to tension was chosen to mimic buckling of the fiber. The stress-strain curves calculated from simulation using the parameters in Table \ref{table:multiscale_parameters} and measured experimentally are shown in Fig.\ \ref{fig:multiscale_properties}(b).
%
...
\blue{Compared to the experimentally measured Poisson ratio ... Describe how Poisson Ratio was measured.}
...
$
\begin{array}{c}
\includegraphics[height=5cm]{figure/AngleLoadingSchematic.pdf} 
\end{array}
$
\end{center}
...
Color maps of the maximum principal strain (MPS) in the neuron structure for a grip strain of 16$\%$ (painful ligament loading) for 0, 45, and 90 degree loading are plotted in Fig.\ \ref{fig:neuron_MPS}.
%
...
\end{figure}
%
For all angles of loading, large portions of the neuron experience strains that are greater than the 16$\%$ grip strain on the surrounding gel. This indicates that the applied grip strains on the collagen gel are amplified in the neuron structure, which could lead to neuronal damage even when the surrounding tissue is moderately loaded. 
...
\end{figure}
%
\blue{axial and transverse strains are considered because ...} In Fig.\ \ref{fig:strain_ccdfs} the shape of the strain distributions vary significantly for different applied grip strains and loading directions. In all cases, the black line right of blue for positive strains and opposite for negative strains. Interestingly, beside the 0 degree loading case, the fraction of neuron volume that experiences 10$\%$ and 16$\%$ strain are identical. 
...
local-strain amplification in the neuron, ccdfs of the MPS in the neuron structure are plotted in Fig.\ \ref{fig:neuron_ccdf_0deg_mps} for 10$\%$ and 16$\%$ bulk strains in the collagen gel.
%
