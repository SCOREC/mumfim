...
The virtual work, $\delta W$, is obtained by multiplying Eq.\ \eqref{eq:multi_cons_mom} by a virtual velocity, $\delta \vec{v}$ (also known as a test function), and integrating over the current volume:
%
\begin{align}
...
\todo{this does not need to be repeated (this might be most general version)}
...
To apply the Newton-Raphson procedure to our problem, the virtual work in Eq.\ \eqref{eq:virtual_work} must be linearized by taking the directional derivative of $\delta W$ with respect to $\delta \vec{v}$ in the direction of $\delta \vec{u}$ at the current coordinates $\vec{x}$
%
\begin{equation}
...
\todo{this needs a derivation/verification}
%% begin comment
%
\begin{eqnarray}
...
\end{eqnarray}
%
where $\vec{e}$ and $\mat{\varepsilon}$ are the Eulerian (or Almansi) strain tensor and small-strain tensor, respectively, and $\delta \vec{d}$ is the virtual rate of deformation tensor: \todo{choice of strain tensor is not consistent with rest of document here}
%
\begin{align}
...
\end{align}
%
where $N_a$ is the $a^{th}$ node of the shape function and $n$ is the number of nodes contained in each shape function. The discretized form of Eq.\ \eqref{eq:virtual_work_linearized} is obtained by applying the approximations in Eq.\ \eqref{eq:discretization} \todo{come back to this...unverified}
%
\begin{eqnarray}
...
\end{equation}
%
are evaluated from calculations at the microscale representative volume elements (RVEs). In order to write the stress derivative in matrix form, Voigt notation is used \todo{swap ordering to match voigt notation}
%
\begin{eqnarray}
[\sigma_{(i)}] \equiv 
\begin{bmatrix}
...
%
%\begin{equation}
%\delta W_{int}(\phi,\delta \vec{v}) = \int_{V_0} \mat{S}:\delta \dot{\mat{E}} dV_0,
%\label{eq:internal_virtual_work_reference}
%\end{equation}
%%
%where $\mat{S}$ and $\delta \dot{\mat{E}}$ are the second Piola-Kirchhoff stress tensor and the virtual material strain rate tensor, respectively. The virtual material strain rate tensor is 
%%
%\begin{equation}
%\delta \dot{\mat{E}} = \frac{1}{2} (\delta \dot{\mat{F}}^T \mat{F} + \mat{F}^T \dot{\mat{F}}).
%\end{equation}
%%
...
%%
%\begin{eqnarray}
%D\delta W_{int} (\phi,\delta \vec{v}) [\delta \vecpmb{u}] &=& \int_{V_0} D(\delta \dot{\mat{E}} : \mat{S}[\vec{u}] dV_0 \nonumber\\
%%
%&=& \int_{V_0} \delta \dot{\mat{E}} : D\mat{S}[\vec{u}] dV_0 + \int_{V_0} \mat{S}:D\delta \dot{\mat{E}}[\vec{u}] dV_0 \nonumber\\
%%
%&=& \int_{V_0} \delta \dot{E}_{ij} \frac{\partial S_{ij}}{\partial E_{kl}} DE_{kl}[\vec{u}] dV_0 + \int_{V_0} S_{ij} \frac{\partial u_j}{\partial X_i} \frac{\partial \delta v_i}{\partial X_j} dV_0 \nonumber\\
%%
%&=&\int_{V_0} D E_{ij} [\delta \vec{v}] \frac{\partial S_{ij}}{\partial E_{kl}} DE_{kl}[\vec{u}] dV_0 + \int_{V_0} S_{ij} \frac{\partial u_j}{\partial X_i} \frac{\partial \delta v_i}{\partial X_j} dV_0,
...
%%
%\begin{align}
%&\delta \dot{\mat{E}} = D \mat{E} [\delta \vec{v}] \nonumber\\
%%
%&DS_{ij}[\vec{u}] = \sum_{k,l=1}^3 \frac{\partial S_{ij}}{\partial E_{kl}}DE_{kl}[\vec{u}] .
%\end{align}
%%
%
...
%%
%\begin{align}
%&D\mat{E}[\delta \vec{v}] = \mat{F}^T \varepsilon \mat{F}; \ \ \mat{\varepsilon} = \frac{1}{2}\left(\nabla \vec{u} + (\nabla \vec{u})^T\right) \nonumber\\
%%
%& D\mat{E}[\vec{u}] = \mat{F}^T \delta \vec{d} \mat{F}; \ \ \delta \vec{d} = \frac{1}{2}\left(\nabla \delta \vec{v}  + (\nabla \delta \vec{v})^T\right) \nonumber\\
%%
%&\mat{S} = J \mat{F}^{-1} \mat{\sigma} \mat{F}^{-T} \nonumber\\
%%
%&\mat{E} = \mat{F}^T \mat{e} \mat{F}; \ \ \mat{e} = \frac{1}{2}(\mat{I} - \mat{b}^{-1}); \ \ \mat{b} = \mat{F}\mat{F}^T \nonumber\\
%%
%&J dV_0 = dV \nonumber\\
%%
%&\nabla_0 \vec{u} = (\nabla \vec{u}) \vec{F} \nonumber\\
%%
%&\nabla_0 \delta \vec{v} = (\nabla \delta \vec{v}) \vec{F}
...
%%
%\begin{eqnarray}
%D\delta W_{int} (\phi,\delta \vec{v}) [\delta \vec{u}] &=& \int_{V_0}  \mat{F}^T \delta \vec{d} \mat{F} : \frac{\partial (J\mat{F}^{-1} \mat{\sigma} \mat{F}^{-T})}{\partial \mat{F}^T \mat{e} \mat{F}} : \mat{F}^T \mat{\varepsilon} \mat{F} dV_0 + \int_{V_0} J \mat{F}^{-1}\mat{\sigma} \mat{F}^{-T} : [\mat{F}^T(\nabla \vec{u})^T (\nabla \delta\vec{v}) \mat{F}] dV_0 \nonumber\\
%%
%&=& \int_{V}  \mat{F}^T \delta \vec{d} \mat{F} : \frac{\partial (\mat{F}^{-1} \mat{\sigma} \mat{F}^{-T})}{\partial \mat{F}^T \mat{e} \mat{F}} : \mat{F}^T \mat{\varepsilon} \mat{F} dV + \int_{V} \mat{F}^{-1}\mat{\sigma} \mat{F}^{-T} : [\mat{F}^T(\nabla \vec{u})^T (\nabla \delta\vec{v}) \mat{F}] dV \nonumber\\
%%
%&=& \int_{V}  \mat{F}^T \delta \vec{d} \mat{F} : \frac{\partial (\mat{F}^{-1} \mat{\sigma} \mat{F}^{-T})}{\partial \mat{F}^T \mat{e} \mat{F}}:\mat{F}^T \mat{\varepsilon} \mat{F} dV + \int_{V} \mat{\sigma} : [(\nabla \vec{u})^T (\nabla \delta\vec{v})] dV \nonumber\\
%\end{eqnarray}
%%
%\red{can also rewrite as ...}
%%
%\begin{equation}
%D\delta W_{int} (\phi,\delta \vec{v}) [\delta \vec{u}] = \int_{V} \delta \vec{d} : \mat{c} : \mat{\varepsilon} dV + \int_{V} \mat{\sigma} : [(\nabla \vec{u})^T (\nabla \delta\vec{v})] dV ,
%\end{equation}
%%
%where
%%
...
%
The derivative of the Cauchy stress is evaluated by using Eq.\ \eqref{eq:macro_stress_discrete} and applying the chain rule \todo{use of $s^{(m)}_{ij}$ not consistent with previous defn.}
%
\begin{eqnarray}
...
\todo{inconsistent...previous part used C and this part uses RVE as superscript}
%
...
\todo{Why is the residual the vector of fiber forces?}
%
\begin{equation}
...
The change of residual with respect to $\vec{x}^{RVE}$ while the interior nodes are fixed is calculated by a geometric argument. To illustrate this calculation, consider a single fiber node that lies on the boundary of the RVE box \todo{not sure if this matrix is correct}
%
\begin{align}
...
%
\begin{equation}
V = \int_{-1}^{1} \int_{-1}^1 \int_{-1}^1 \text{det}\mat{J}^e d\xi_1 d\xi_2 d\xi_3  = \int_{-1}^{1} \left(\int_{-1}^1 \left(\int_{-1}^1 \text{det}\mat{J}^e d\xi_1 \right)d\xi_2\right) d\xi_3 ,
\label{eq:volume}
\end{equation}
%
...
%
\begin{equation}
V = \sum_{i=1}^{N_{gp}} \sum_{j=1}^{N_{gp}} \sum_{k=1}^{N_{gp}} W_i W_j W_k \text{det}\mat{J}^e(\xi_1^{(i)},\xi_2^{(j)},\xi_3^{(k)})
\label{eq:gauss_integration}
\end{equation}
%
where $N_{gp}$ is the number of Gauss integration points, $W_i$ is the weight of the $i^{th}$ integration point, $\mat{J}^e$ is the Jacobian matrix, and $\xi_i^{(j)}$ is the $i^{th}$ component of the $j^{th}$ integration point. Note that $V$ is the volume for a hexahedron where $N_{gp}=2$, $W_i=1$, and $\xi_i^{(j)}=\pm 1/\sqrt{3}$. 
...
The matrix $[\partial \vec{x}^{RVE}/\partial \vec{u}^k]$ has dimensions of 24 (8 RVE corner nodes $\times$ 3 dofs per node) $\times$ 12 (4 FE tetrahedron nodes $\times$ 3 dofs per node) for each element: 
%
