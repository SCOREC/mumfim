...
namespace bio
{
  ...
  NeoHookeanIntegrator(NonlinearTissue * n,
    ...
    void inElement(apf::MeshElement * me)
    {
      ...
      // current coordinate field w.r.t. current coordinate mesh elements elements
    ...
    void atPoint(apf::Vector3 const &p, double w, double dV)
    {
      ...
      */
      ...
      for (int i = 0; i < dim; i++)
      {
        for (int j = 0; j < dim; j++)
        {
          Cauchy[i][j] = mu/detF*(leftCauchyGreen(i,j)-(i==j))+lambda/detF*log(detF)*(i==j);
        }
      }
      ...
      // E_G = 1/2(C-I), C=F^T.F
      apf::Matrix3x3 greenStrain(0.5*(rightCauchyGreen(0,0)-1),0.5*rightCauchyGreen(0,1),0.5*rightCauchyGreen(0,2),
                                 0.5*rightCauchyGreen(1,0),0.5*(rightCauchyGreen(1,1)-1),0.5*rightCauchyGreen(1,2),
                                 0.5*rightCauchyGreen(2,0),0.5*rightCauchyGreen(2,1),0.5*(rightCauchyGreen(2,2)-1));
